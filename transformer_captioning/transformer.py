...
class AttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        dot_product = (torch.matmul(query, key_t)) / np.sqrt(self.embed_dim)  # scaled dot product
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        output = x + self.encoding(torch.arange(S, dtype=torch.long).to(x.device))
...
class SelfAttentionBlock(nn.Module):
    ...
    def forward(self, seq, mask):
        ...
        residual_connection = self.self_attn(seq, seq, seq, mask)
        residual_dropout = self.dropout(residual_connection)
        out = self.layernorm(residual_dropout + seq)
        return out
...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(self, seq, cond):
        ...
        cross_attention = self.cross_attn(seq, cond, cond) #query,key,val
        residual_with_dropout = seq + self.dropout(cross_attention)
        out = self.norm(residual_with_dropout)
...
class FeedForwardBlock(nn.Module):
    ...
    def forward(self, seq):
        ...
        residual_connection_with_og = seq + self.dropout(self.mlp(seq))
        out = self.norm(residual_connection_with_og)
        return out
